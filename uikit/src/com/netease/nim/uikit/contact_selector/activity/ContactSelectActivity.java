...
public class ContactSelectActivity extends UI implements View.OnClickListener, android.support.v7.widget.SearchView.OnQueryTextListener {
    ...
    public static final int RESULT_CHOSESCHOOL = 201;

    public static final int RESULT_CHOSECLASS = 202;

    public static ContactSelectActivity context;
...
//    private SearchView searchView;
    ...
    private View schoolLay,classLay;

    private TextView selectedAccsTV;

    private ArrayList<String> selectedAccounts;

    private DcContactSelectCloseListener closeListener;

    public static List<ContactEntity> contactList;
...
//        searchView.setOnQueryTextListener(this);
    ...
    public void setOnSelectCloseListener(DcContactSelectCloseListener listener) {
                ...
                if (option.multi) {
                    ...
                    if (contactAdapter.isSelected(position)) {
                        ...
                        if (contact != null) {
                            ...
                            contactListRemove(contact.getContactId());
                        }
                    } else {
                        if (contactSelectedAdapter.getCount() <= option.maxSelectNum) {
                            ...
                            if (contact != null) {
                                ...
                                contactListAdd(contact.getContactId(),contact.getDisplayName());
                            }
                        } else {
...
//                        }
        ...
        //TODO 跳转选择组织结构和班级
        schoolLay = findView(R.id.schoolLay);
        schoolLay.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent("com.zdhx.im.contactselect");
                intent.putExtra("kind",0);
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                        | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED
                        | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivityForResult(intent,RESULT_CHOSESCHOOL);
            }
        });
        classLay = findView(R.id.classLay);
        classLay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                        | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED
                        | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivityForResult(intent,RESULT_CHOSECLASS);
            }
        });
        if(!option.multi) {
    ...
    private void initContactSelectArea() {
        ...
        selectedAccsTV = (TextView)findView(R.id.selectedAccsTV);
...
//               count = contactSelectedAdapter.getCount() - 1;
//            } else {
...
//            notifySelectAreaDataSetChanged();
    ...
    private String getOKBtnText(int count) {
        ...
        return caption + " (" + (count < 1 ? 0 : count) + ")";
    }
            ...
            if (option.allowSelectEmpty || checkMinMaxSelection(contacts.size())) {
                ...
                if (contactList != null && contactList.size() != 0) {
                    for (ContactEntity contactEntity : contactList) {
                        selectedAccounts.add(contactEntity.getContactId());
                    }
                }
    ...
    public void onSelectChanged() {
        //TODO 去重
            ...
            if (option.allowSelectEmpty || checkMinMaxSelection(contacts.size())) {
                ...
                for (IContact c : contacts) {
                    selectedAccounts.add(c.getContactId());
                    nameList.add(c.getDisplayName());
                }
                if (contactList != null && contactList.size() != 0) {
                    for (ContactEntity contactEntity : contactList) {
                        if (StringUtil.isEmpty(contactEntity.getContactId())) {
                            return;
                        }
                        if (!selectedAccounts.contains(contactEntity.getContactId().trim())) {
                            selectedAccounts.add(contactEntity.getContactId());
                            nameList.add(contactEntity.getName());
                        }
                    }
                }
                ...
                btnSelect.setText(getOKBtnText(selectedAccounts.size()));
                selectedAccsTV.setText(names);
            }
    }

    public void contactListAdd(String accId,String name) {
        if (contactList != null) {
            boolean key = true;
            for (ContactEntity contactEntity : contactList) {
                if (contactEntity.getContactId().equals(accId)) {
                    key = false;
                    break;
                }
            }
            if (key) {
                ...
                ce.setName(name);
                ce.setContactId(accId);
                contactList.add(ce);
            }
        }
    }

    public void contactListRemove(String accId) {
        if (contactList != null && contactList.size() != 0) {
            for (int i = 0; i < contactList.size(); i++) {
                if (contactList.get(i).getContactId().equals(accId.trim())) {
