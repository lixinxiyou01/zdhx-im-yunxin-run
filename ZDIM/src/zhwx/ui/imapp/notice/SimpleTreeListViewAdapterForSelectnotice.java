...
public class SimpleTreeListViewAdapterForSelectnotice<T> extends TreeListViewAdapter<T> {
	...
	public SimpleTreeListViewAdapterForSelectnotice(ListView tree, SelectActivity context,
	...
	public View getConvertView(Node node, int arg0, View view, ViewGroup arg2) {
		...
		if(!node.isLeaf()){
			if(SeletePageOneActivity.parentPositionMap.get(node.getId())!=null){
				holder.checkBox1.setChecked(true);
			}else{
				holder.checkBox1.setChecked(false);
			}
...
//		context.getTopBarView().setRightButtonText(context.getString(R.string.radar_ok_count, context.getString(R.string.dialog_ok_button1) , ContactListFragmentForSelect1.positionMap.size()));
	...
	public void onItemClick(Node node) {
		if(node.isLeaf()){
			...
			if(SeletePageOneActivity.parentPositionMap.get(node.getId()) == null){
				SeletePageOneActivity.parentPositionMap.put(node.getId(), node);
				addChildNode(node);
				contentAll(node);
			}else{
				SeletePageOneActivity.parentPositionMap.remove(node.getId());
				removeChildNode(node);
				removeUperNode(node);
			}
		}
		SelectActivity.okBT.setText("确定\n" + SendNewNoticeActivity.positionMap.size());
