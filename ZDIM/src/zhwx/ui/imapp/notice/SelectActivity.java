...
public class SelectActivity extends BaseActivity {
	...
	private SimpleTreeListViewAdapterForSelectnotice<FileBean> treeListViewAdapter;
		...
		searchLV.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position,
				...
				if (SendNewNoticeActivity.positionMap.get(classes.get(position).getId()) != null) {
					SeletePageOneActivity.remove(classes.get(position).getId());
				} else {
					Node node = new Node();
	...
	private void getData(int kind) {
		...
		if(kind == KIND_TEACHER) {
			url = UrlUtil.getTeacherTreeForNative(ECApplication.getInstance().getAddress(),map);
			titleTV.setText("教师");
		} else if (kind == KIND_STUDENT) {
			url = UrlUtil.getStudentTreeForNative(ECApplication.getInstance().getAddress(),map);
			titleTV.setText("学生");
		} else if (kind == KIND_PARENT) {
			url = UrlUtil.getParentTreeForNative(ECApplication.getInstance().getAddress(),map);
			titleTV.setText("家长");
		} else if (kind == KIND_GROUP) {
			url = UrlUtil.getTreeOfMyGroup(ECApplication.getInstance().getAddress(),map);
			titleTV.setText("常用联系人");
		}
	...
	public void refreshTree(String json) {
					...
					for (int j = 0; j < tree.get(i).getUserList().size(); j++) {
						...
						datas.add(new FileBean(Integer.parseInt(user.getId().length() < 5 ? "0" : user.getId().substring(
								25, 31)), Integer.parseInt(tree.get(i).getId()
										...
										.getId(), PinYinUtil.getNameMatchString(user.getName())));
					}	
				}
			}
		}
		...
		try {
			treeListViewAdapter = new SimpleTreeListViewAdapterForSelectnotice<FileBean>(
					mListView, (SelectActivity) context, datas, 0);
	...
	protected int getLayoutId() {
		return R.layout.contacts_select_activity_notice;
	}
}
