 ...
 */
public class GoodsListForHandActivity extends BaseActivity implements OnClickListener {
	...
	private void initView() {
		...
		mystoreLV.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
				...
				GrantByHandActivity.putGoods(allDataList.get(position));
	...
	public class OrderListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			boolean key = false; 
			for (int i = 0; i < GrantByHandActivity.selectedGoodList.size(); i++) {
				if(GrantByHandActivity.selectedGoodList.get(i).getId().equals(getItem(position).getId())) {
					key = true;
					break;
				}
			}
