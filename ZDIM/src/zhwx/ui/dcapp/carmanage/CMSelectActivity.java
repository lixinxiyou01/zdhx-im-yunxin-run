...
import zhwx.ui.dcapp.carmanage.adapter.SimpleTreeListViewAdapterForCM;
...
public class CMSelectActivity extends BaseActivity {
	...
	private SimpleTreeListViewAdapterForCM<FileBean> treeListViewAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		searchLV.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position,
					long id) {
				if (OrderCarActivity.positionMap.get(classes.get(position).getId()) != null) {
					OrderCarActivity.positionMap.remove(classes.get(position).getId());
				} else {
					Node node = new Node();
	...
	public void refreshTree(String json) {
		...
		try {
			treeListViewAdapter = new SimpleTreeListViewAdapterForCM<FileBean>(
					mListView, (CMSelectActivity) context, datas, 0);
	...
	public class myAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if(OrderCarActivity.positionMap.get(getItem(position).getId())!=null){
	...
	protected int getLayoutId() {
		return R.layout.contacts_select_activity_cm;
	}
}
