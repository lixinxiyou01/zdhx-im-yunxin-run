...
import zhwx.ui.settings.ChangePasswordActivity;
...
public class UserProfileSettingActivity extends UI implements View.OnClickListener {
    ...
    private void findViews() {
        ...
        ((TextView) password_layout.findViewById(R.id.attribute)).setText(R.string.password);
     ...
     * 上传头像到IM
     */
    public void upLoadImage(final File image) {
        map = new HashMap<String, ParameterValue>();
        map.put("id", new ParameterValue(ECApplication.getInstance() .getCurrentIMUser().getId()));
        loginMap = (HashMap<String, ParameterValue>) ECApplication.getInstance().getLoginMap();
...
//                            if (upLoadFlag.contains("uploadeFail")) {
